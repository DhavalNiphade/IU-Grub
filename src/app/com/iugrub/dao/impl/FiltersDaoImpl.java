...
public class FiltersDaoImpl implements FiltersDaoAPI{
	...
	public void updateObject() {
		...
		Session objSession = objSessionFactory.openSession();
		Transaction tx = objSession.beginTransaction();
		Query objQuery = objSession.createQuery("from FoodStore f where f.strStoreName = ?");
		objQuery.setParameter(0, "The TRAVELLER");
		List<FoodStore> resultList = objQuery.list();
		FoodStore objStore = resultList.get(0);
		...
		objSession.saveOrUpdate(objStore);
		tx.commit();
		objSession.close();
