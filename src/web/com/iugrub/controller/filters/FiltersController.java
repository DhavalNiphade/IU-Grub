...
public class FiltersController {
	...
	public static Map<String,Set<FoodStore>> mapCampusNameToStore = new HashMap<String,Set<FoodStore>>();
	...
	public void getAllData(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		//objFiltersService.updateObject();
		...
		for(FoodStore objStore : listFoodStores) {
			...
			if(mapCampusNameToStore.containsKey("Indiana Memorial Union")) {
				
				mapCampusNameToStore.get("Indiana Memorial Union").add(objStore);
			} else {
				
				mapCampusNameToStore.put("Indiana Memorial Union", new HashSet<FoodStore>());
				mapCampusNameToStore.get("Indiana Memorial Union").add(objStore);
			}
	...
	public void getFilteredOffers(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String[] strCampusFilters = request.getParameterValues("CAMPUS_SELECTION[]")==null? new String[1] :request.getParameterValues("CAMPUS_SELECTION[]");
		List<String> listCampusFilters;
		if(request.getParameterValues("CAMPUS_SELECTION[]")==null) {
			
			listCampusFilters = new ArrayList();
		} else {
			listCampusFilters = Arrays.asList(strCampusFilters);
		}
		
		if(listCampusFilters.contains("Indiana Memorial Union")) {
	...
	@RequestMapping(value = "/filters/updateStoreFilters")
	public void updateStoreFilters(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(request.getParameterValues("CAMPUS_SELECTION[]")==null) {
			...
			listFilters = Arrays.asList(strCampusFilters);
		}
	
		// call a service method to get search results
		StringBuilder objResponse = new StringBuilder();
		for(String strCampusName : listFilters) {
			
			if(mapCampusNameToStore.containsKey(strCampusName)) {
			Set<FoodStore> setStore = mapCampusNameToStore.get(strCampusName);
			for(FoodStore objStore : setStore) {
